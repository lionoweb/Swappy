...
class mailer {
    ...
    private $folderb = FOLDER_;
	private $url_site = URL_SITE;
    //ENVOIE MAIL
...
<body style="width:100% !important; background-color:#FFFFFF; -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%; height:100%;">';
    ...
    function send_remind($hash, $mail, $name, $cname) {
        ...
        $html .= '';
        $html .= 'Bonjour '.$cname.' ('.$name.')<br><br>Merci de bien vouloir changer le mot de passe de votre compte en cliquant sur ce lien :<br><a href="'.$this->url_site.$this->folderb.'inscription.php?remind='.$hash.'" target="_blank">'.$this->url_site.$this->folderb.'inscription.php?remind='.$hash.'</a><br><br><i>Si vous ne pouvez pas cliquer sur ce lien, copiez le et collez-le lien dans votre navigateur.</i></p>';
    ...
    function newmessage($email, $date, $idc, $cname, $oname, $oid) {
        ...
        $message = '<h4>Bonjour {CNAME}</h4>';
        $message .= '<p>Vous avez reçu un nouveau message de la part de <a href="'.$this->url_site.$this->folderb.'profil-{OID}.php">{ONAME}</a> le {DATE}<br><br><a href="'.$this->url_site.$this->folderb.'messagerie.php#select-{CID}">Cliquez ici pour le consulter</a>.<br><br><i>Vous pouvez à tout moment désactiver les notations par mail de vos messages sur votre <a href="'.$this->url_site.$this->folderb.'profil.php">page profil</a>.</i>';
    ...
    function send_validation_rdv($owner, $user, $servid, $servname, $date, $cc) {
        ...
        $message = '<h4>Bonjour {CNAME}</h4>';
        $message .= '<p>Votre rendez-vous a été validé auprès de vous et de votre interlocuteur.<br><br><u>Voici donc le récupitulatif :</u><br>Il s\'agit d\'un rendez-vous avec <a href="'.$this->url_site.$this->folderb.'profil-{OID}.php">{ONAME}</a>  pour {YOUR} service \'<a href="'.$this->url_site.$this->folderb.'annonce-{SERVID}.php">{SERVNAME}</a>\'.<br> Ce rendez-vous est prévu pour le {DATE}.<br><br><i>Vous pouvez toujours annuler ce rendez-vous en cliquant sur lien envoyé dans votre <a href="'.$this->url_site.$this->folderb.'messagerie.php#select-{IDCONV}">messagerie</a></i>';
        $first = preg_replace("/\{CNAME\}/", $owner->fullname, $message);
        $first = preg_replace("/\{OID\}/", $user->ID, $first);
        $first = preg_replace("/\{ONAME\}/", $user->fullname, $first);
        ...
        $first = preg_replace("/\{CNAME\}/", $user->fullname, $message);
        $first = preg_replace("/\{OID\}/", $owner->ID, $first);
        $first = preg_replace("/\{ONAME\}/", $owner->fullname, $first);
