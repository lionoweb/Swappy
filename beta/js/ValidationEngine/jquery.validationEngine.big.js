...
alert("ok");
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "equals":
						...
						break;
					case "equalsPASS":
					errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._equalsPASS);
					break;
					case "timeCheck":
					errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._timeCheck);
					break;
					case "funcCall":
		...
		*/
		_handleStatusCssClasses: function(field, options) {
		 ...
		 _validityProp: {
			 ...
			 "equalsPASS": "pattern-mismatch",
			 "timeCheck": "pattern-mismatch",
		...
		*/
		...
		_equalsPASS: function(field, rules, i, options) {
			var equalsField = rules[i + 1];

			if (field.val() != $("#" + equalsField).val())
				return options.allrules.equalsPASS.alertText;
		},
		_timeCheck: function(field, rules, i, options) {
			var equalsField = rules[i + 1];
			var ss = (field.val()).split(":");
			var hourend = ss[0];
			var minend = ss[1];
			equalsField = equalsField.replace(/\}/gi, "]").replace(/\{/gi, "[");
			var ss_ = ($("input[name='" + equalsField+"']").val()).split(":");
			var hourstart = ss_[0];
			var minstart = ss_[1];
			if(hourstart <= hourend) {
				if(((hourstart == hourend) && (minend <= minstart))) {
					...
					var result = true;
					if($("select[name='dispoday" + num +"']").val() == "all") {
						 $("input[name^='dispostart']").each(function(index, element) {
                            if($(this).attr("name") != equalsField) {
								n = $(this).attr("name").replace("dispostart", "");
								hours = $(this).val().split(":")[0];
								houre = $("input[name='dispoend" + n +"']").val().split(":")[0];
								mins = $(this).val().split(":")[1];
								mine = $("input[name='dispoend" + n +"']").val().split(":")[1];
								if((hours<hourstart && houre<hourend) || (hours>hourstart && houre>hourend && hours>hourend)) {
									//OK
								} else {
									//PROBLEM
									if(hours == hourstart && mins>minstart) {
									} else if(hours == hourend && mine > minstart) {
									} else if(hourstart == houre && mine <= minstart && hourend <= houre) {
									} else {
										result = false;
										return false;
									}
					...
					} else if($("select[name='dispoday" + num +"']").val() == "weekend") {
						$("input[name^='dispostart']").each(function(index, element) {
                            if($(this).attr("name") != equalsField && ($(this).parent().find("select").val() == "sam" || $(this).parent().find("select").val() == "dim" || $(this).parent().find("select").val() == "all" || $(this).parent().find("select").val() == "weekend")) {
								n = $(this).attr("name").replace("dispostart", "");
								hours = $(this).val().split(":")[0];
								houre = $("input[name='dispoend" + n +"']").val().split(":")[0];
								mins = $(this).val().split(":")[1];
						...
						var va = $("select[name='dispoday" + num +"']").val();
						if(va == "sam" || va == "dim") {
							$("input[name^='dispostart']").each(function(index, element) {
								if($(this).attr("name") != equalsField && ($(this).parent().find("select").val() == va || $(this).parent().find("select").val() == "all" || $(this).parent().find("select").val() == "weekend")) {
									n = $(this).attr("name").replace("dispostart", "");
									hours = $(this).val().split(":")[0];
									houre = $("input[name='dispoend" + n +"']").val().split(":")[0];
									mins = $(this).val().split(":")[1];
							...
							$("input[name^='dispostart']").each(function(index, element) {
								if($(this).attr("name") != equalsField && ($(this).parent().find("select").val() == va || $(this).parent().find("select").val() == "all")) {
									n = $(this).attr("name").replace("dispostart", "");
									hours = $(this).val().split(":")[0];
									houre = $("input[name='dispoend" + n +"']").val().split(":")[0];
									mins = $(this).val().split(":")[1];
		...
		_calculatePosition: function (field, promptElmt, options) {
			...
			switch (positionType) {
				...
				case "centerLeft":
					...
					break;
					case "center":
					promptleftPosition = (fieldLeft) + (promptElmt.width() / 2) + 2;
					promptTopPosition = fieldTop+4;
					marginTopSize = 0;
					break;
				case "bottomLeft":
